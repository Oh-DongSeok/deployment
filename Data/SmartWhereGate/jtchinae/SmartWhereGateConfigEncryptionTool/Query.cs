namespace FXKIS.SmartWhere.Gate
{
    ...
    public class Query
    {
        ...
        private const string QueryCreateSmartWhereGate =
            @"
            -- --------------------------------------------------------
            --
            ...
            INSERT INTO `GATE_ENVIRONMENT` (IS_ENABLED) VALUES (1);
        ...
        public static bool CreateGateDatabase ()
        {
            Global.SecretKeyForCryptography = Query.SecretKeyForCryptography;

            Global.LoadServiceConfigurationFromFile();

            using (MySqlConnection conn = Global.CreateDatabaseConnectionWithoutDatabase())
            {
                if (conn == null)
                {
                    return false;
                ...
                {
                    cmd.Connection  = conn;
                    cmd.CommandText = Query.QueryCreateSmartWhereGate;
