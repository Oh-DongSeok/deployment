            ...
            try
            {
                ...
                try
                {
                    using (PrnExtractor extractor = new PrnExtractor(spool.SpoolFile))
                    {
                        ...
                        PrnExtractedData data = extractor.ExtractHeavy(this.Environment.Database, this.Environment);                                                                        
                        ...
                        //mailSender.SendEmail(this.Environment.Email.DebugBody,emailTo);
                        ...
                        if (extractor.emailTo != null || extractor.emailTo != string.Empty)
                        {
                            //EmailSender mailSender = new EmailSender(this.Environment);
                            
                            //mailSender.SendEmail(EmailEnvironment.MailType.SpoolComplete, extractor.emailTo,extractor.originalDocumentName);
                        }
                ...
                while (this.CancelToken.IsCancellationRequested == false) 
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            // timeout 
                            if (threadAnalyzed.CreateTime.IsOverThanNow(0, 0, this.Environment.Analyzer.AnalyzeTimeoutMinHeavy, 0) == true)
                            {
                                try
                                {
                                    // Move a spoolfile to ErrSpool directory
