 ...
 * 프린트중 팝업
 */
var ScanningPopup = new TemplatePage();

ScanningPopup.ID = "pop_scanningPopup";

/**
 * 개별 페이지의 Data정의
 */
ScanningPopup._initModel=function()
{
	this._data=
	{
		buttonList:[
		...
		imageList:[
		           {id:"img_scanning_progress_bar", src:Img.PROGRESS_BAR} //2016.05.16 chone 프린팅 화면 결제 금액 표시 개선 요청
		],
		textList:[
		          {id:"lbl_scanning_progress", text:Msg.ScanningPopup.lblScanning} //2016.05.12 chone 프린팅 화면 결제 금액 표시 개선 요청
		]
	};
	this._imgListSetting();//imageList 설정
};

ScanningPopup._imgListSetting = function(){
...
ScanningPopup._onPageChange = function(){
...
ScanningPopup._onPageLeave = function(){
 ...
 */
ScanningPopup.updateDisplay = function(){
	var obj = document.getElementById(ScanningPopup.ID);
	...
	if(glbInfo.screenType == BrowserLib.ScreenType.WSVGA_ALL_SCREEN){
		Common.setPosition("lyr_SP_wrapper", "left", "112px");
	}
	...
	//Common.changeVisibility("lbl_scanning_progress", "none");
	return;
...
ScanningPopup.updatePriceInfo = function()
{
	Common.changeVisibility("lbl_scanning_progress", "block");
 ...
 */
ScanningPopup.setAnimation_CampainImg = function(){
 ...
 * 프린팅 애니메이션
 */
ScanningPopup.setAnimation_Default = function(){
	var i = 0;
 ...
 * 프린팅 애니메이션 해제
 */
ScanningPopup.clearAnimation = function(){
 ...
 */
ScanningPopup.EventHandler = function(event, id)
{
	switch(event){
		case "onbuttonup":
