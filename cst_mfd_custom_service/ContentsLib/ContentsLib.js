...
var ContentsLib = {
	...
	/**
	 * カスタムサービススクリプトライブラリ(CSLib)のバージョンを表す<br>
	 * ポーズキーを押下するとBeep音が鳴動する<font color="#ff0000">(V3.3.xまで)</font>
	 * @constant
	 * @type String
	 * @default null
	 * @lang ja
	 */
	/**
	 * CSLib version. Represents Custom Service script library (CSLib) vesion<br>
	 * When Pause key is pressed, beep sound is sounded <font color="#ff0000">(until V3.3.x)</font>
	 *	@constant
	 * @type String
	 * @default null
	 * @lang en
 ...
 * <br>
 * 本関数のコール後は、ポーズキーを5秒以上押下することでコンテンツ保守情報画面がポップアップ表示される。<br>
 * 画面上の閉じるボタン押下でコンテンツの画面に戻る。<br>
 * <br>
 * <font color="#ff0000">制限事項:</font><br>
 * 保守情報画面表示が行われた場合、以降のコンテンツ動作は保証しない。
 *
 * @addon
 * @static
 * @lang ja
 ...
 * WebDAVLib.setEventListener()<br>
 * <br>
 * After this function is called, Custom Service Information screen is displayed as popup when the Pause key is pressed for five or more seconds.<br>
 * If the Close button on the top of the screen is pressed, the screen returns to the previous screen.<br>
 * <br>
 * <font color="#ff0000">Restrictions:</font><br>
 * If the Custom Service Information screen is displayed, the operation of the contents is not guaranteed after that.
 *
 * @addon
 * @static
 * @lang en
...
/**
 * ログを書き込む<br>
 * 書きこまれたログは、コンテンツ情報表示画面のポップアップ表示中に"*"キーを押下することで表示される(デバッグモード時のみ)。<br>
 * コンテンツをクローズするとwriteLogで書きこまれたログは削除される。<br>
 * コンテンツのクローズ後もログを保持したい場合は<a href="./LogLib.html">LogLib</a>を使用すること。
 ...
 */
 /**
 * Writes log.
 * The written log is displayed by pressing "*" key while Custom Service Information popup is displayed (only in debug mode).<br>
 * When the application is closed, logs written using writeLog are deleted.<br>
 * To retain the log after closing the application, use <a href="./LogLib.html">LogLib</a>.
 * @param {String} logMessage
 *
 * @addon
 * @static
...
ContentsLib._onHardKeyEvent = function(keyCode)
{
	...
	// ポーズを一定期間押下で情報表示
	// ポーズキー押下時のBeep音鳴動はV3.3.xまで。それ以降は鳴動しない。
	if (keyCode == BrowserExt.keyCode.FX_VK_PAUSE) {
		// AR155824 2012.3.26 removed by A.Orimoto
...
ContentsLib._onInfoPopup = function(event, id)
{
	...
	if (event && event.type == "mouseup") {
		...
		case "maintenance_close_button":
			...
			//press状態を解除
			WidgetLib.setWidgetStatus(buttonId, {press:false});
	...
	if(event && event.type == "mousedown") {
		...
		case "maintenance_close_button":
			//press状態に変更
			WidgetLib.setWidgetStatus(buttonId, {press:true});
			return;
		default:
			//ここに落ちるケースは想定していない
			break;
...
ContentsLib._registerInfoPopup = function()
{
	...
	var closeBtnTop = 0;
	var scrHigh = 400;
	var bgSrc = "./ContentsLib/Base_VerInfo_800-480.png";	// 800x480 WVGA Screen
	if ( _scrType && _scrType == BrowserLib.ScreenType.WVGA_STATUS_MESSAGE ){
		...
		scrHigh = 520;
		bgSrc = "./ContentsLib/Base_VerInfo_800-600.png";
	}
	...
	_backElem.setAttribute("draggable", "false");
	...
	if (ContentsLib.contentsIcon) {
		...
		_iconElem.setAttribute("draggable", "false");
	...
	_titleElem.style.width = "500px";	// width추가
	...
	_idElem.style.width = "200px";		// width추가
	...
	_verElem.style.width = "300px";		// width추가
	...
	_nameElem.style.width = "600px";	// width추가
	...
	_crElem.style.width = "600px";		// width추가
	...
	if(ContentsLib.trackingId || ContentsLib.serialNumber)
	{
		...
		_stElem.style.width = "300px";	// width추가
	...
	_buttonElem.style.top = (scrHigh - closeBtnTop)+ "px";
	...
	_imgElem.setAttribute("draggable", "false");
...
//	_textElem.style.left = "40px";
	...
	/* イベントを受けるコールバック関数を書き換える(AR156527) */
	/* キャプチャリングフェーズでイベントを処理するためaddEventListenerを使う*/
	_buttonElem.addEventListener("mousedown", ContentsLib._onInfoPopup, true);
	_buttonElem.addEventListener("mouseup", ContentsLib._onInfoPopup, true);
...
ContentsLib._registerLogPopup = function()
{
	...
	/* イベントを受けるコールバック関数を書き換える(AR156527) */
	/* キャプチャリングフェーズでイベントを処理するためaddEventListenerを使う*/
	_buttonElem.addEventListener("mousedown", ContentsLib._onLogPopup, true);
	_buttonElem.addEventListener("mouseup", ContentsLib._onLogPopup, true);
	WidgetLib.registerWidget(_popupElem);
};
