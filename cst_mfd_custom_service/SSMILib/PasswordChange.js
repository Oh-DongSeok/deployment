 ...
 * @lang en
 */
SSMILib.ChangePasswordInfo = function()
{
	this.userId = "";
	this.userNewPw = "";
};
 ...
 * @private
 */
Extend(SSMILib.ChangePasswordInfo.prototype, XMLLib.SOAPMsg.prototype);

/**
 * @private
 */
SSMILib.ChangePasswordInfo.prototype.createMsg = function()
{
	...
	itNode = xml.createElement("userNewPw");
	itNode.appendChild(xml.createTextNode(this.userNewPw));
	sdNode.appendChild(itNode);
 ...
 * @param	{SSMILib.PasswordChangeInfo}	sdObj	文書取得用のオブジェクトを指定する
 ...
 * @param	{SSMILib.PasswordChangeInfo}	sdObj	Object for retrieving document information
 * @param	{Int}	timeout		Communication timeout
 * @addon
 * @static
 * @lang en
 */
SSMILib.ChangePassword = function(sdObj, timeout, remoteHost)//2017.11 KIS [v1.5] SmartUI 복수의 Print Server 대응 refs #4502
{
	//SmartUI 2.0 Enhance 2015.05 Start
	//var url = glbConfig.remoteHost + "RegisterCard";
	var url = remoteHost + "ChangePassword";//2017.11 KIS [v1.5] SmartUI 복수의 Print Server 대응 refs #4502
	...
	if(timeout){
		req.timeout = timeout;
		req.ontimeout = SSMILib.ChangePasswordInfo.gsdErrorCb;
	}
        ...
        if(this.readyState === 4){
        	if(this.status === 200){
        		...
        		if(resObj && typeof resObj.status != "undefined"){
        			SSMILib.onEvent("ChangePassword", true, resObj);
        		}else{
        			SSMILib.ChangePasswordInfo.gsdErrorCb();
        		}
	        }else{
	        	SSMILib.ChangePasswordInfo.gsdErrorCb();
	        }
 ...
 * @private
 */
SSMILib.ChangePasswordInfo.gsdSuccessCb = function(res)
{
	KISUtil.debug("ChangePasswordInfo/gsdSuccessCb", res.responseText);
	var resObj = null;
	var _result = false;

	if(!res.responseXML) {
		SSMILib.onEvent("ChangePassword", _result, null);
		return;
	}
	...
	SSMILib.onEvent("ChangePassword", _result, resObj);
};
 ...
 */
//SmartUI 2.0 Enhance 2015.05 Start
SSMILib.ChangePasswordInfo.gsdErrorCb = function()
{
	SSMILib.onEvent("ChangePassword", false, null);
};
//SmartUI 2.0 Enhance 2015.05 End
