...
Common.onLoadBody = function() {
    ...
    if (flg_Dummy_Beep) {
        ...
        glbConfig.chkFaxUse = p_glbConfig.chkFaxUse;
        glbConfig.chkFax = p_glbConfig.chkFax;
    ...
    function quoteParentModule() {
        ...
        glbConfig.PRINTER_IP = (!flg_Dummy_Beep) ? p_glbConfig.PRINTER_IP : '192.168.200.133';
    }
};
...
Common.onLoadEvent = function(event, id) {
    switch (event) {
        case "GetCustomizeDeviceInfo":
            ...
            if (glbConfig.chkFaxUse != glbConfig.chkFax) {
                var msg = { title: Msg.Page.WarnPopup.title, message: Msg.ERROR_MSG.FAIL_COMM };
                WarnPopup._param = { type: "startup", title: msg.title, message: msg.message, targetPage: false };
                PageManager.changePage(WarnPopup, PageManager.type.NORMAL);
            }
...
Common.doChargePrice = function() {
    ...
    var info = {
        "service": SERVICE_CODE.FAX_PRICE,
        "docInfo": {
            "resolution": res,
            "pages": glbInfo.ScanImageInformation.Impressions.toString(),
            "colormode": "1", // 0/1 (Color/Black&White) Fax는 Only B/W
            "fileName": fileName,
            "fileType": glbFileTran.docFormat // ToDo: 차후 변수로 변경해야함.
        },
        "userId": "",
...
Common.doCharge = function(payType) {
    ...
    var charge = {
        "service": SERVICE_CODE.FAX_CHARGE,
        "docInfo": {
            "resolution": res,
        ...
        "userInfo":{
            "userId":"",
            "faxNumber": glbUserInfo.faxNumber,
            "telNumber": glbUserInfo.telNumber,
            "informingConsent": true
        },
