...
AdminSettingPage._initModel = function()
{
	this._data =
	{
		...
		textList:[
			...
			,{id : "lbl_LP_isCreditUse", text: "신용카드 사용여부"}
			,{id : "lbl_LP_isImoneyUse", text: "i-money 사용여부"}
...
AdminSettingPage.EventHandler = function(event, id)
{
	...
	switch(event){
		case "SetCsvLocalData":
			if(arguments[1] == true){
				LogLib.info("[CS-RM-xxxx0]writeFile Before");
				this.writeFile("./data/data.js", this.createDataStr(this.setDataFileString()));
				LogLib.info("[CS-RM-xxxx0]fixedValue After");
			}else{
		...
		case "onbuttonup":
			...
			switch(id)
			{
				case "btn_LP_confirm":
					...
					LogLib.info("[CS-RM-xxxx0]fixedValue is "+fixedValue);
