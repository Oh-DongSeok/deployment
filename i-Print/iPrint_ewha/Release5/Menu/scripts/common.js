	...
	switch(event){
		...
		case "GetTcpIpInfo"://2016.09.28 KIS Chone 복합기 IP 자동 취득 기능의 추가 refs #4065
            if(arguments[1] == true){
                ...
                glbConfig.PRINTER_IP = tcpInfoObj[0].IPAddress;
		...
		case "GetTrayInfo":
			if (arguments[1] == true) {
				...
				for(var i=0,iMax=4;i<iMax;i++){//TODO 현재 Tray4까지만 존재하는걸로 가정. 이후 트레이장착을 인식후, 트레이 장착수만큼 표시하도록 처리필요.
					traySize = Common.paperSizeMsg(arguments[2][i].MediumSize); // size 이외의 문자열 제거 및 지원 용지만 확인 20221006
...
//4
		...
		case "ListCsv":
        	if(flg_Dummy_Beep||arguments[1]==true){
				...
				glbInfo.CSInfo = convertCsInfos(arguments[2]);
			...
			function setAvailableCS(_csInfo,_services){
			...
			function changeFormatForExcute(obj,tmp){
				return {changeDisplay:"cstm"+lpad(obj.idx,2,"0"), iframe:obj.url, key:tmp.key, type:tmp.name};
			}
...
/**
 * 서버로부터 메뉴서비스 표시가 완료된 Response함수
 */
Common.DoPureLoginChkResp = function(){
	...
	if(data.status=='success'){
		...
		var targetPopup = ServiceSelectPage;//서비스선택페이지로 천이
	}else{
...
// Tray 용지 정보에서 사이즈만 추출
Common.paperSizeMsg = function(size)
{
	var result = 'A4';
	if(size.indexOf('A4') != -1){
		result = 'A4';
	}else{
		if(size.indexOf('A3') != -1){
			result = 'A3';
		}else{
