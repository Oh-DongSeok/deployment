...
SystemEventLib.onSystemEvent = function (event){
	...
	switch ( _attrName ){
	  case "AAAAccountState" :
		...
		if ( _category == "All" ) {
			_obj = getAuthObj(_attrVal);
			SystemEventLib.onEvent("AuthEvent", _obj);
		}
		if ( _category == "Acc" ) {
			_obj = getCstmAuthAccObj(_attrVal);
	  ...
	  case "AAAAuthenticationState" :
		...
		if ( _client == "Xcp" ) {
			_obj = getCstmAuthXcpObj(_attrVal);
			SystemEventLib.onEvent("CstmAuthAccEvent", _obj);
		} else {
		}
		break;
	  ...
	  case "ChangeScreenResult" :
		_obj = getChgScreenResultObj(_attrVal);
 ...
 * @private
 */
function getCstmAuthAccObj(attrValue){
...
function getCstmAuthXcpObj(attrValue){
	var _retObj = new Object();
	for ( var prop in attrValue ){
		switch ( prop ) {
		  case "State" :
 ...
 * @private
 */
function getChgScreenResultObj(attrValue){
