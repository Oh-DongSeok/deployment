	...
	changePage:function(newPage,type){
		...
		if(this.currentPage!=null)
		{
			if(newPage)
			{
				...
				{
					if(!Common.contains([ErrorPopup.ID, WarnPopup.ID, WaitingPage.ID], this.currentPage.ID)){
						this.prevPage = this.currentPage;
						this.history.push(this.currentPage.ID);
					}
					//전전 페이지가 basePage인지 확인
					var basePageObjs = document.querySelectorAll(".basePage.page");
					for(var i=0,len=basePageObjs.length;i<len;i++){
						basePageObjs[i].className = "page";
					}
		...
		if(type == this.type.NORMAL || type == this.type.CONFIRM || type == this.type.MODAL || type == this.type.ROLLBACK)
		{
