 ...
 * @param {String} source コピー先にコピーする任意の文字列
 * @example 
 * ■使用例 
 ...
 * @param {String} source A given character string copied to the destination (target) node
 * @example 
 * EXAMPLE: 
 *		var from = "//" + XMLLib.NSPREFIX.JT + ":Scan/" + XMLLib.NSPREFIX.JT + ":ColorMode";
 *		var to = "//" + XMLLib.NSPREFIX.JT + ":Invoke/" + XMLLib.NSPREFIX.JT + ":Profile/" + XMLLib.NSPREFIX.JT + ":Description";
 ...
 * @lang en
 */
JFLib.Assign = function(from , to, append, back, source)
{
	/**
	 ...
	 */
	/**
	 * A given character string copied to the destination (target) node
	 * @type String
	 * @default null
	 * @lang en
	 */
...
JFLib.Assign.prototype.toXmlNode = function (xml) 
{
	...
	if(!this.from && this.source){
		source.appendChild(xml.createTextNode(this.source));
	}else{
