...
ResultPopup._initModel=function()
{
	...
	{
		buttonList:[
				   {id:"btn_thankyou_screen_close", type:WidgetLib.ButtonType.NORMAL, attr:{targetImgId :"img_thankyou_screen_close", offImg: Img.ICN_CF_CONFIRM_OFF, pressImg: Img.ICN_CF_CONFIRM_PRESS}}
		],
		imageList:[
		           //{id:"thankyou_screen_base",src:Img.BG.BG_RESULT},
				   {id:"eco_level1",src:Img.BG_RESULT.TREE},
				   {id:"eco_level2",src:Img.BG_RESULT.TREE},
				   {id:"eco_level3",src:Img.BG_RESULT.TREE}
		],
		textList:[
				   {id:"lbl_thankyou_screen_close",	text:Msg.Common.lbl_POPUP_close},
		]
	};
};
...
ResultPopup._onPageChange = function(){
	...
	var ecoNum=0;
	for(i in glbDataSet.selectedDoc){
		ecoNum = ecoNum + calcEcoSheetsNum(glbDataSet.selectedDoc[i].pageCnt, outputSheetCnt(i), glbDataSet.selectedDoc[i].printCnt);//단면출력용지수,실제출력용지수,부수
	}
...
ResultPopup.displayEcoImage = function(){
	...
	switch(glbInfo.thankScreenState){
		case THANK_SCREEN_STATE.DAY:
			img_base = Img.BG_RESULT.DAY0;
			img_add = Img.BG_RESULT.DAY;
			pos_x = "470px";
			pos_y = "250px";
			break;
		case THANK_SCREEN_STATE.EVENING:
			img_base = Img.BG_RESULT.EVE0;
			img_add = Img.BG_RESULT.EVE;
			pos_x = "470px";
			pos_y = "250px";
			break;
		case THANK_SCREEN_STATE.NIGHT:
			img_base = Img.BG_RESULT.NIGHT0;
			img_add = Img.BG_RESULT.NIGHT;
			pos_x = "10px";
			pos_y = "100px";
			break;
	}
...
ResultPopup.displayEcoLevel = function(){
	var eco_level=[];
	for(var i in glbDataSet.selectedDoc){
		eco_level.push(calcEcoLevel(i));
	}
	var max = Math.max.apply(null,eco_level);
	...
	switch(event){
		case "GetChildJobInfo":
			if(arguments[1] == true){
				...
				var sheets = childJobInfo.PrintMediumInformation.Sheets;//실제 출력된 용지수
				var copies = childJobInfo.PrintMediumInformation.Copies;//부수
