...
var JobMonitor = (function() {
        ...
        if (event == "GetJob") {
            if (result) {
                if (obj && _jobID == obj.JobID) {
                    KISUtil.debug("_onSSMIEvent", "# " + obj.JobID + "/" + obj.Status + "/" + _poolingCnt);
                    if (!_jobFound) {
                        _jobFound = true;
                    ...
                    switch (obj.Status) {
                        ...
                        default:
                            /*
                            if(obj.FlowServiceJobInformation.TemplateName){     // 해당 오브젝트가 없는 경우가 있음.(원고 규격이 맞지않았을 경우에 발생)
                                if (obj.FlowServiceJobInformation.TemplateName != "Copy(Scan)") {
                                    _poolingCnt--;
                                    if (_jobID) setTimeout("JobMonitor.getJobInfo('" + _jobID + "');", _poolingTerm);
                                } else {
                                    if (_jobID) setTimeout("JobMonitor.getJobInfo('" + _jobID + "');", _poolingTerm);
					...
					switch(obj.Status){
						...
						case "canceledByUser":
						case "canceledBySystem":
						//case "pending": //출력시에 팬딩이 발생할 수 있음 Processing과 동일하게 처리하도록 함.
						case "terminating":
							_stopMonitor(obj);
                                LogLib.write("[CS] job Type : print, job status : " + obj.Status, LogLib.LEVEL.INF);
                                //2016.12.07 KIS Chone #4032 프린트 환불 처리 타이밍 변경(에러 팝업에서 로그아웃 버튼 누른 시점으로 이동)
                        ...
                        case "unknown":
							//_stopMonitor(obj);
							_jobID = glbInfo.printJobID = obj.JobID;
							//_jobInfo.listType = null;
							//_jobInfo.sortKeys = null;
							setTimeout("JobMonitor.getJobInfo('" + _jobID + "');", _poolingTerm);
							break;
						default:
							if(_jobID){
								_jobInfo.listType = SSMILib.JOBLISTTYPE.COMPLETED;
								setTimeout("JobMonitor.getJobInfo('" + _jobID + "');", _poolingTerm);
							}else{
    ...
    function _startMonitor(jobObj, _cnt) {
        ...
        if (_registJobInfo(jobObj) && !_isPoolingMode) {
            _jobFound = false;
            //_poolingCnt = _cnt || 240; // Default치를 5분으로 설정
            _poolingCnt = 1300;
            _isPoolingMode = true;
            _doPooling(jobObj);
        }
    ...
    function _doPooling(_jobObj) {
        ...
        if (_poolingCnt > 0) {
            if (!_jobInfo) {
				...
				_jobInfo.sortKeys = [{element : SSMILib.JOBDETAIL.STARTTIME, order : SSMILib.SORTORDER.DESC}];
            }
