...
function TemplatePage(){
	...
	this._initImage = function()
	{
		...
		for (var i = 0, il = this._data.imageList.length; i < il; i++) {
			...
			var imgNode = document.getElementById(obj.id);
			if(!imgNode || !obj.src || imgNode.src == obj.src){
				//console.log("id=" + obj.id + " : src=" + obj.src);
			}
			else{
				imgNode.setAttribute("src", obj.src);
			}
			if (obj.className) {
				imgNode.className = obj.className;
			}
		}
	}; 
	...
	this._initPullDown = function(key){
		...
		for(var i = 0, il = pulldownList.list.length; i < il; i++) {
			...
			document.body.appendChild(Common.createHTMLPulldown(obj.pageKey, obj.key, obj.value.length,false));
