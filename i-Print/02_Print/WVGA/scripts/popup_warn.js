...
WarnPopup._initModel=function()
{
	this._data = {
		buttonList:[
			{id:"btn_WP_close",	type:WidgetLib.ButtonType.NORMAL, attr:{targetImgId:"img_WP_close", offImg:Img["BTN_ERROR_OFF"], pressImg:Img["BTN_ERROR_PRESS"], disableImg:Img["BTN_ERROR_DIS"]}}
		],
		imageList:[
		],
		textList:[
					...
					if(this._param){
						if(this._param.type==="logout"){
							...
							if(this._param.detail == "refund"){
								WidgetLib.setWidgetStatus("btn_WP_close", {enable:false});//2017.02.02 FXKIS 환불 통신 실패(http status 0 발생 등)시 리트라이 실시 refs #4191
